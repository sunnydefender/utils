...
/**
 * long 64位: 48位日期戳(yyyy-MM-dd HH:mm:ss), 5位serverIdBit, 4位seqBit, 7位后缀(分库表)
 * 每豪秒间隔容纳id数: 16
 * @author Administrator
 *
 */
public class SuffixIdImpl implements SuffixId {
	...
	private static final long SUFFIX_MASK = 0x7fL;
	private static final long TIMESTAMP_LEFT_MOVE_BITS = 16L;
    private static final long SERVER_ID_LEFT_MOVE_BITS = 11L;
	private static final long SEQUENCE_LEFT_MOVE_BITS = 7L;

	public SuffixIdImpl(long serverId) {
		this.serverId = serverId;
		serverIdBits = (this.serverId & SERVER_ID_MASK) << SERVER_ID_LEFT_MOVE_BITS;
		sequenceBits = 0L;
	}
	
	@Override
	public synchronized long createId(String suffix) {
        ...
        long suffixId = Long.valueOf(suffix) & SUFFIX_MASK;
		
		return (timestampBits << TIMESTAMP_LEFT_MOVE_BITS)
				| (sequenceBits << SEQUENCE_LEFT_MOVE_BITS) 
				| serverIdBits 
				| suffixId;
	}
